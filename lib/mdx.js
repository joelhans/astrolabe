...
export async function getContent(source) {
  ...
  const content = await Promise.all(
    files.map(async (filepath) => {
      ...
      // const md = await renderToString(content, { MDXComponents, scope: data });
...
export async function fetchFiles(dir) {
  console.log(`${root}${dir}/**/*.{md,mdx}`)
  return glob.sync(`${root}${dir}/**/*.{md,mdx}`)
}
